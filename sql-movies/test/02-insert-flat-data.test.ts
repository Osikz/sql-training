  ...
  return (
    `insert into movies (imdb_id, popularity, budget, budget_adjusted, revenue, 
      revenue_adjusted, original_title, homepage, tagline, overview, runtime, release_date) VALUES` +
       movies.map(movie => `('${escape(movie.imdbId)}', ${movie.popularity}, ${movie.budget},
       ${movie.budgetAdjusted}, ${movie.revenue}, ${movie.revenueAdjusted}, '${escape(movie.originalTitle)}',
       '${escape(movie.homepage)}', '${escape(movie.tagline ? movie.tagline : ' ')}', '${escape(movie.overview)}',
